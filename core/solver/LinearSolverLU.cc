...
namespace csim
{
    ...
    int LinearSolverLU::solve(csimModel::MComplex *A, unsigned int n, csimModel::MComplex *x, csimModel::MComplex *B)
    {
        if (m_bufRows != n)
        {
            m_bufRows = n;
        ...
        if (A)
        {
             ...
             */
            memcpy((void*)m_bufA, A, m_lenRow * n);
            ...
            for (unsigned int c = 0; c < n; c++)
            {
                ...
                {
                    /* m_bufA r[c] <-> r[prvot] */
                    memcpy((void*)m_bufRow, &m_bufA[c * n], m_lenRow);
                    memcpy((void*)&m_bufA[c * n], &m_bufA[pivot * n], m_lenRow);
