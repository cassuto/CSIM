...
namespace csim
{
    ...
    int MNAOptimizer::singularRow(Complex *A, unsigned curRow, unsigned int nRows)
    {
        ...
        if (getGminEnabled())
        {
            if (curRow < m_netlist->getNumNodes())
            {
                ...
                {
                    std::cout << "A tiny resistor is inserted between the node" << curRow << " and ground." << std::endl;
                    m_lastFixRow = curRow;
                }
            }
        }
